...
$orgId = 'org_2bae74f86c454cfaafafc1ea2f52a1fd'
...
try {
  ...
  if ($wranglerExitCode -ne 0) { throw "wrangler D1 insert failed with exit code $wranglerExitCode" }
} finally {
  if ($null -ne $oldPreference) { $ErrorActionPreference = $oldPreference }
  Pop-Location
}
