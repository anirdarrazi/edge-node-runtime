...
try {
  & '..\node_modules\.bin\wrangler.CMD' d1 execute autonomousc-edge-network-db --remote --file $sqlPath *> (Join-Path $nodeDir 'create-node-output.txt')
} finally {
  Pop-Location
}
...
try {
  ...
  python -m node_agent.vast_smoke `
    ...
    --json-indent 2 *> (Join-Path $nodeDir 'vast-launch-output.json')
