...
function headers(extra = {}) { return { authorization: `Bearer ${apiKey}`, "content-type": "application/json", ...extra }; }
async function requestJson(path, init = {}) {
  const response = await fetch(`https://batchrouter.com${path}`, init);
  const text = await response.text();
  let payload;
  try { payload = text ? JSON.parse(text) : null; } catch { payload = { parse_error: true, text: text.slice(0, 2000) }; }
...
function manifest() {
  return {
    ...
    metadata: { fleet_test: true, fleet_run_id: runId, requested_item_count: size, provider_under_test: "autonomousc" },
...
const body = manifest();
const quote = await requestJson("/v1/batches/quote", { method: "POST", headers: headers(), body: JSON.stringify(body) });
...
while (Date.now() - started <= timeoutMs) {
  ...
  console.log(JSON.stringify({ event: "poll", size, batch_id: batchId, state: latest?.state, counts, polls, elapsed_seconds: Math.round((Date.now()-started)/1000) }));
...
if (latest?.state !== "completed" || Number(counts.failed ?? 0) !== 0) process.exitCode = 1;
